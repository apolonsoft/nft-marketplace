...
const published = ["CreatorCollectionFactory", "CreatorERC721", "CreatorERC1155", "MarketplaceSettlement", "TransparentUpgradeableProxy", "ProxyAdmin"];

if (!existsSync(out)) execFileSync("forge", ["build"], { cwd: forgeRoot, stdio: "inherit" });
mkdirSync(abiDir, { recursive: true });
for (const name of published) {
  const source = readdirSync(out, { withFileTypes: true }).map((entry) => entry.name).find((entry) => existsSync(join(out, entry, `${name}.json`)));
