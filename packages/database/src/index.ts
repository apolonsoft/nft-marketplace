...
CREATE TABLE IF NOT EXISTS worker_media_assets (asset_id text PRIMARY KEY, source_url text NOT NULL, declared_mime text NOT NULL, actual_mime text, byte_size bigint, checksum text, kind text NOT NULL, status text NOT NULL DEFAULT 'PENDING', error_message text, provider_id text, cid text, attempts integer NOT NULL DEFAULT 0, next_retry_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS worker_media_previews (preview_key text PRIMARY KEY, asset_id text NOT NULL, mime_type text NOT NULL, byte_size bigint NOT NULL, width integer NOT NULL, height integer NOT NULL, checksum text NOT NULL, status text NOT NULL DEFAULT 'READY', created_at timestamptz NOT NULL DEFAULT now());
`;
