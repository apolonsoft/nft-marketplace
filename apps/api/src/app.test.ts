...
describe('API bootstrap', () => {
  ...
  beforeAll(async () => {
    ...
    const module = await Test.createTestingModule({ imports: [AppModule] })
      ...
      .overrideProvider(INDEXER_PROBE)
      .useValue({ check: async () => ({ status: 'up', details: { stalenessMs: 0 } }) })
      .compile();
  ...
  it('serves versioned REST with request correlation', async () => {
  ...
  it('serves liveness and readiness separately', async () => {
    ...
    expect((await request(app.getHttpServer()).get('/health/ready')).body.status).toBe('ready');
    databaseUp = false;
    expect((await request(app.getHttpServer()).get('/health/ready')).status).toBe(503);
    expect((await request(app.getHttpServer()).get('/health/live')).status).toBe(200);
    databaseUp = true;
  });
  it('exposes metrics and OpenAPI documentation', async () => {
    expect((await request(app.getHttpServer()).get('/metrics')).text).toContain(
      'api_http_requests_total',
    );
    expect(
      (await request(app.getHttpServer()).get('/docs/openapi.json')).body.paths,
    ).toHaveProperty('/api/v1/system');
  });
  it('generates and serves GraphQL', async () => {
    const response = await request(app.getHttpServer())
      .post('/graphql')
      .send({ query: '{ apiVersion }' });
    expect(response.body).toEqual({ data: { apiVersion: 'v1' } });
  });
