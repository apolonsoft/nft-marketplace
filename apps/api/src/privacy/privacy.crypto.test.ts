...
const config = { privacy: { encryptionKey: 'a-development-key-that-is-long', keyVersion: 'v1' } } as ApiConfig;
describe('PrivacyCrypto', () => {
  ...
  it('rejects malformed ciphertext', () => expect(() => new PrivacyCrypto(config).decrypt('bad')).toThrow());
});
