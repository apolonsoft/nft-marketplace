...
const repo: MarketplaceReadRepository = {
  query: async () => [
...
describe('MarketplaceReadService', () => {
  it('filters stale rows and encodes opaque cursors', async () => {
    ...
    await expect(service.page('listings', { after: 'bad' })).rejects.toMatchObject({
      code: 'VALIDATION_ERROR',
    });
  });
  it('rejects pending data and invalid ranges', async () => {
    const service = new MarketplaceReadService(repo);
    await expect(service.page('nfts', { includePending: true })).rejects.toMatchObject({
      code: 'FORBIDDEN',
    });
    await expect(service.page('nfts', { minPrice: '10', maxPrice: '1' })).rejects.toMatchObject({
      code: 'VALIDATION_ERROR',
    });
  });
});
