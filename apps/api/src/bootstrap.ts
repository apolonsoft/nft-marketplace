...
export async function createApplication() {
  ...
  app.use('/graphql', (request: any, _response: any, next: any) => {
    ...
    const points = query.length + (query.match(/\b(items|nodes|edges|listings|sales|activity)\b/g) ?? []).length * 25;
    if (depth > 20 || points > 5000) { _response.status(400).json({ code: 'VALIDATION_ERROR', message: 'GraphQL query complexity exceeds the configured budget', retryable: false }); return; }
    next();
  });
