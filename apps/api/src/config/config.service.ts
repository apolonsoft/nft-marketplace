...
const list = (value: string | undefined, fallback: string) =>
  (value ?? fallback)
    ...
    .map((item) => item.trim())
    .filter(Boolean);
const chainIds = (value: string | undefined) =>
  list(value, '31337').map((item) => positiveInteger(item, 31337, 'SIWE_ALLOWED_CHAIN_IDS'));
...
export class ConfigService {
  load(source: NodeJS.ProcessEnv = process.env): ApiConfig {
    ...
    const privacyKey =
      source.PRIVACY_ENCRYPTION_KEY ??
      (nodeEnv === 'production' ? undefined : 'development-only-key-change-me');
    if (!['development', 'test', 'production'].includes(nodeEnv))
      throw new Error('NODE_ENV must be development, test, or production');
    if (nodeEnv === 'production' && (!source.DATABASE_URL || !source.INDEXER_HEALTH_URL))
      throw new Error('DATABASE_URL and INDEXER_HEALTH_URL are required in production');
    if (
      nodeEnv === 'production' &&
      (!source.AUTH_ACCESS_PRIVATE_KEY || !source.AUTH_ACCESS_PUBLIC_KEY)
    )
      throw new Error(
        'AUTH_ACCESS_PRIVATE_KEY and AUTH_ACCESS_PUBLIC_KEY are required in production',
      );
    if (!privacyKey || privacyKey.length < 16)
      throw new Error('PRIVACY_ENCRYPTION_KEY must be at least 16 characters');
    return {
      ...
      indexerMaxStalenessMs: positiveInteger(
        source.INDEXER_MAX_STALENESS_MS,
        60_000,
        'INDEXER_MAX_STALENESS_MS',
      ),
      ...
      auth: {
        ...
        nonceTtlSeconds: positiveInteger(
          source.SIWE_NONCE_TTL_SECONDS,
          600,
...
export const loadApiConfig = (source: NodeJS.ProcessEnv = process.env) =>
  new ConfigService().load(source);
