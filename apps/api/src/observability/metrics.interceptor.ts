...
export class MetricsInterceptor implements NestInterceptor {
  constructor(
    ...
    @Inject(ApiLogger) private readonly logger: ApiLogger,
  ) {}
  intercept(context: ExecutionContext, next: CallHandler) {
    ...
    return next.handle().pipe(
      ...
      finalize(() => {
        ...
        this.metrics.duration.observe(labels, durationSeconds);
        this.logger.structured.info('HTTP request completed', {
          ...labels,
          durationSeconds,
...
export class MetricsController {
  ...
  @Get() async get(@Res() response: Response) {
    response.type(this.metrics.registry.contentType).send(await this.metrics.registry.metrics());
  }
}
