...
const factory = (process.env.FACTORY_ADDRESS ??
  (manifest.contracts as Record<string, { address: `0x${string}` }>).CreatorCollectionFactory
    ?.address) as `0x${string}` | undefined;
const settlement = (process.env.SETTLEMENT_ADDRESS ??
  (manifest.contracts as Record<string, { address: `0x${string}` }>).MarketplaceSettlement
    ?.address) as `0x${string}` | undefined;
if (!factory || !settlement)
  throw new Error(
    `Deployment manifest for ${network} must contain factory and settlement addresses`,
  );
...
export default createConfig({
  ...
  networks: {
    [network]: { chainId: manifest.chainId, transport: http(rpcUrl), pollingInterval: 1_000 },
  },
  contracts: {
    Factory: {
      abi: CreatorCollectionFactoryAbi,
      network: network as 'anvil' | 'baseSepolia',
      address: factory,
      startBlock,
    },
    Settlement: {
      abi: MarketplaceSettlementAbi,
      network: network as 'anvil' | 'baseSepolia',
      address: settlement,
      startBlock,
    },
    ERC721: {
      abi: CreatorERC721Abi,
      network: network as 'anvil' | 'baseSepolia',
      address: [] as `0x${string}`[],
      startBlock,
    },
    ERC1155: {
      abi: CreatorERC1155Abi,
      network: network as 'anvil' | 'baseSepolia',
      address: [] as `0x${string}`[],
      startBlock,
    },
