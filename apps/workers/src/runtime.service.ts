...
export class WorkerRuntimeService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly mediaStore = new MediaStore(
    new Pool({ connectionString: this.config.databaseUrl }),
  );
  ...
  async onModuleInit() {
    const queue = new Queue(QUEUES.webhook, {
    ...
    const worker = new Worker(QUEUES.webhook, async (job) => this.processWebhook(job), {
      connection: this.connection,
      concurrency: this.config.concurrency,
      settings: {
        backoffStrategy: (attemptsMade: number) =>
          backoffMs(attemptsMade + 1, this.config.backoffBaseMs, this.config.backoffMaxMs),
      },
    });
    worker.on('active', (job) =>
      this.metrics.jobs.inc({ queue: job.queueName, job: job.name, outcome: 'started' }),
    );
    worker.on('completed', (job) =>
      this.metrics.jobs.inc({ queue: job.queueName, job: job.name, outcome: 'completed' }),
    );
    ...
    const eventQueue = new Queue(QUEUES.events, {
      connection: this.connection,
      defaultJobOptions: defaultJobOptions(this.config),
    });
    this.queues
      .set(QUEUES.events, eventQueue)
      .set(
        deadLetterQueue(QUEUES.events),
        new Queue(deadLetterQueue(QUEUES.events), { connection: this.connection }),
      );
    const eventWorker = new Worker(
      QUEUES.events,
      async (job) => {
        const outcome = await this.eventStore.process(job.data, {
          duplicate: () => this.metrics.events.inc({ outcome: 'duplicate' }),
          stale: () => this.metrics.events.inc({ outcome: 'stale' }),
          processed: () => this.metrics.events.inc({ outcome: 'processed' }),
        });
      },
      {
        connection: this.connection,
    ...
    for (const queueName of Object.values(MEDIA_QUEUES))
      this.queues.set(
        queueName,
        new Queue(queueName, {
          connection: this.connection,
    ...
    for (const queueName of Object.values(MEDIA_QUEUES))
      this.queues.set(
        ...
        new Queue(deadLetterQueue(queueName), { connection: this.connection }),
      );
    ...
    const previewWorker = new Worker(
      MEDIA_QUEUES.preview,
      async (job) => {
        const data = job.data as { assetId: string; body: string; checksum: string };
        await this.mediaStore.preview(
          data.assetId,
    ...
    const pinWorker = new Worker(
      ...
      async (job) => {
        const data = job.data as { assetId: string; body: string; checksum: string };
        const result = await this.ipfs.pin(Buffer.from(data.body, 'base64'), data.assetId);
        await this.mediaStore.status(data.assetId, {
  ...
  private async relay(queue: Queue) {
    const result = await this.eventStore.pool.query(
      'SELECT id,event_type,payload_version,payload,deduplication_key,chain_id,block_number,block_hash,transaction_hash,log_index FROM event_outbox WHERE published_at IS NULL ORDER BY block_number,log_index LIMIT $1',
      [this.config.relayBatchSize],
    );
    for (const row of result.rows) {
      const payload = typeof row.payload === 'string' ? JSON.parse(row.payload) : row.payload;
      const event = domainEventJobSchema.parse({
        eventId: row.id,
      ...
      await this.eventStore.pool.query(
        'UPDATE event_outbox SET published_at=$1 WHERE id=$2 AND published_at IS NULL',
        [Date.now(), row.id],
      );
    }
  }
  ...
  private async processWebhook(job: Job): Promise<void> {
    ...
    try {
      const result = await deliver(
        parsed.data.event as WebhookDeliveryJob['event'],
        parsed.data.subscription,
      );
      if (!result.ok && result.retryable)
        throw new Error(`Retryable webhook response: ${result.status ?? 'network error'}`);
      if (!result.ok) throw new Error(`Terminal webhook response: ${result.status}`);
    } finally {
      this.metrics.duration.observe(
        { queue: job.queueName, job: job.name },
  ...
  private async onFailed(job: Job | undefined, error: Error) {
    ...
    await dlq.add(
      ...
      { jobId: `${job.queueName}:${job.id}`, removeOnComplete: { age: 604_800 } },
    );
    this.metrics.jobs.inc({ queue: job.queueName, job: job.name, outcome: 'quarantined' });
    this.logger.error('Job quarantined', {
      queue: job.queueName,
      jobId: job.id,
      attempts: job.attemptsMade,
      error: error.message,
