...
for (const file of manifests) {
  ...
  for (const dep of Object.keys(deps)) {
    ...
    const isApp = file.startsWith('apps/');
    const targetIsApp = names.get(dep).startsWith('apps/');
    if ((!isApp && targetIsApp) || (isApp && targetIsApp))
      errors.push(`${file}: forbidden dependency ${dep}`);
  }
}
if (errors.length) {
...
function walk(dir) {
  return readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
    ...
    if (entry.isDirectory() && !['node_modules', '.turbo', 'dist', 'build'].includes(entry.name))
      return walk(path);
...
function resolveSource(file, specifier) {
  ...
  for (const candidate of [
    ...
    try {
      ...
      /* unresolved imports are handled by workspace tooling */
    }
  }
  return undefined;
}
