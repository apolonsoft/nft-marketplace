...
const manifest = join(resolve(marketplaceRoot, "../.."), "packages", "contracts", "deployments", `${profile === "base-sepolia" ? "base-sepolia" : "anvil"}.json`);
if (process.env.BROADCAST === "true" && process.env.REDEPLOY !== "true" && existsSync(manifest)) {
  ...
  if (Object.keys(existing.contracts ?? {}).length > 0) throw new Error(`Deployment manifest already contains contracts. Set REDEPLOY=true to intentionally deploy a new set: ${manifest}`);
}
...
execFileSync("forge", args, { cwd: new URL("..", import.meta.url), stdio: "inherit", env: process.env });
